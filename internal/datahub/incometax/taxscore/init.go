...
func SetupInit(apiGroup fiber.Router, cfg *application.Config, client httpclient.HTTPClient) {
	...
	taxComplianceGroup.Post("/single-request", middleware.Auth(), middleware.ValidateRequest(taxScoreRequest{}), middleware.GetJWTPayloadFromCookie(cfg), controller.SingleSearch)
