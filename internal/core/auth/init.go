...
func SetupInit(authAPI fiber.Router, cfg *application.Config, client httpclient.HTTPClient, mailSvc *mail.SendMailService) {
	...
	authAPI.Post("/register-member", middleware.GetJWTPayloadFromCookie(cfg), middleware.AdminAuth(), middleware.ValidateRequest(member.RegisterMemberRequest{}), controller.RegisterMember)
	...
	authAPI.Post("/logout", middleware.GetJWTPayloadFromCookie(cfg), controller.Logout)
	authAPI.Post("/refresh-access", middleware.GetPayloadFromRefreshToken(cfg), controller.RefreshAccessToken)
	authAPI.Put("/send-email-activation/:email", middleware.GetJWTPayloadFromCookie(cfg), middleware.AdminAuth(), controller.RequestActivation)
